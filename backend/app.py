...
def callback(ch, method, properties, body):
    ...
    if 'action' not in request:
        ...
        if action == 'GETHASH':
            ...
            cursor.execute(
                'SELECT hash FROM usersinfo WHERE username=%s;', (username,))
            row = cursor.fetchone()
            if row == None:
            ...
            cursor.execute(
                'SELECT * FROM usersinfo WHERE email=%s or username=%s;', (email, username))
            ...
            if searchby == 'cocktail_name':
                ...
                r = requests.get(
                    'https://www.thecocktaildb.com/api/json/v1/1/search.php', params=searchbycocktailname)
                r = r.json()
                if r['drinks']:
                    ...
                    for i in range(1, 16):
                        if r['drinks'][0]['strIngredient' + str(i)]:
                            cocktailingredients.append(
                                r['drinks'][0]['strIngredient' + str(i)])
                    for i in range(1, 16):
                        if r['drinks'][0]['strMeasure' + str(i)]:
                            cocktailmeasurements.append(
                                r['drinks'][0]['strMeasure' + str(i)])
                    cocktailinstructions = r['drinks'][0]['strInstructions']
                    cocktailcategory = r['drinks'][0]['strCategory']
                    response = {'success': True, 'search': 'cocktail_name', 'cocktailname': cocktailname, 'cocktailimage': cocktailimage, 'cocktailingredients': cocktailingredients,
                                'cocktailmeasurements': cocktailmeasurements, 'cocktailinstructions': cocktailinstructions, 'cocktailcategory': cocktailcategory}
                else:
                    response = {'success': False,
                                'message': "Invalid Cocktail Name"}
            elif searchby == 'ingredient_name':
                searchbyingredientname = {'i': data['searchfield']}
                r = requests.get(
                    'https://www.thecocktaildb.com/api/json/v1/1/search.php', params=searchbyingredientname)
                r = r.json()
                if r['ingredients']:
                    ...
                    response = {'success': True, 'search': 'ingredient_name', 'ingredientname': ingredientname,
                                'ingredientdescription': ingredientdescription, 'ingredienttype': ingredienttype, 'ingredientalchohol': ingredientalchohol}
                else:
                    response = {'success': False,
                ...
                cocktailinstructions = r['drinks'][0]['strInstructions']
                cocktailcategory = r['drinks'][0]['strCategory']
                response = {'success': True, 'search': 'random', 'cocktailname': cocktailname, 'cocktailimage': cocktailimage, 'cocktailingredients': cocktailingredients,
                            'cocktailmeasurements': cocktailmeasurements, 'cocktailinstructions': cocktailinstructions, 'cocktailcategory': cocktailcategory}
        else:
